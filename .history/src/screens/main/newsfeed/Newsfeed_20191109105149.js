...
class NewsFeed extends Component {
    ...
    changeStatusLike = async (id) => {
        ...
        await this.state.isActiveLike ? this.addLike(id): this.deleteLike(id)
    }
    // changeStatusComment = () => {
    ...
    async deleteLike(id){
        try{
          const asyncStorage = await AsyncStorage.getItem("token")
          const like = await axios.post(Constant.rootAPI+'/deleteLike/'+id,
          {headers: {'Authorization': `Bearer ${asyncStorage}`}})
        } catch(err) {
          throw new Error(err)
        }
                              ...
                              <CardItem>
                                <Left>
                                  ...
                                  onPress={() => this.actionLike(item.id)}>
                                    <Icon name="heart" size={22} color={(this.state.isActiveLike && this.state.idLikeOfPost == item.id) ? colors.primaryColor : colors.darkGray}/>
