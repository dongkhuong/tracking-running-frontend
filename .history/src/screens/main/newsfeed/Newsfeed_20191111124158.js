...
class NewsFeed extends Component {
    ...
    async addLike(id){
        try{
          ...
          if(like.data.business_code == 1){
            await this.setState({idLikeOfPost: id})
    ...
    async countLikes(id){
        try{
          ...
          if(countLikes.data.business_code == 1){
            await this.setState({userLikes: countLikes.data.data.userLikes, sumOfLikes: countLikes.data.data.sumOfLikes})
            console.log(this.state.userLikes)
            console.log(this.state.sumOfLikes)
          } else if (like.data.business_code == 0) {
            await this.setState({sumOfLikes: 0})
    ...
    async componentDidMount () {
        ...
        await this.countLikes("55f2d1f00e874492a667253312bad7c3")
                              ...
                              <CardItem>
                                <Left>
                                  ...
                                  onPress={() => this.actionLike(item.id)}>
                                    <Icon name="heart" size={22} color={(this.state.idLikeOfPost == item.id || this.state.arrLikes.includes(item.id)) ? colors.primaryColor : colors.darkGray}/>
