...
class NewsFeed extends Component {
    ...
    countComments = async (post_id) => {
        try{
          const asyncStorage = await AsyncStorage.getItem("token")
          const numberOfComments = await axios.get(Constant.rootAPI+'/countComments/'+post_id,
          {headers: {'Authorization': `Bearer ${asyncStorage}`}})
          console.log(numberOfComments.data.number_of_comments)
        } catch(err) {
          throw new Error(err)
        }
    }
                              ...
                              <CardItem>
                                ...
                                <Body style={{alignItems: 'flex-start'}}>
                                  ...
                                  onPress={() => this.props.navigation.navigate('ListComment',{comment_id: item.id})}>
                                    ...
                                    <Text style={{fontSize: 11, marginLeft: 5}}>{this.countComments(item.id)} Comments</Text>
                                  </TouchableOpacity>
                                </Body>
                                <Right>
                                  <TouchableOpacity 
