...
import { View, Text, ScrollView, TouchableOpacity, Image, ToastAndroid, RefreshControl, Alert, ImageBackground } from 'react-native'
...
import { Avatar, Button, Card, Title, Paragraph, IconButton, ActivityIndicator } from 'react-native-paper'
...
class ClubDetail extends Component {
    ...
    _onRefresh = () => {
        ...
        this.showPostInGroup().then(() => this.setState({refreshing: false}))
    }
        ...
        return (
            <ScrollView
               ...
               <View style={{height: metrics.DEVICE_HEIGHT*0.3}}>
                    <TouchableOpacity>
                        ...
                        {this.props.navigation.getParam('image') != null ?
                            <ImageBackground source={{ uri: this.props.navigation.getParam('image')}} resizeMethod={'resize'} resizeMode={'cover'} style={{height: '100%', width: '100%'}}/>:
                            <Image source={require('../../../assets/images/mapdemo.jpg')} resizeMethod={'resize'} resizeMode={'cover'} style={{height: '100%', width: '100%'}}/>}
                    </TouchableOpacity>
               </View>
               <View style={{alignItems: 'center', backgroundColor: colors.white}}>
                    ...
                    <View style={{width: metrics.DEVICE_WIDTH*0.8, marginBottom: 10}}>
                        {this.state.data.description != null?<HTML tagsStyles={{p: {textAlign: 'center'}}} html={this.state.data.description} />:null}
                    </View>
               ...
               {this.state.isShowButtonJoin ? (
                ...
                <View style={{flexDirection: 'row', flex:1, alignItems: 'center', paddingVertical: 10, backgroundColor: colors.white}}>
                    <View style={{flex: 1, alignItems: 'center'}}>
                        <TouchableOpacity onPress={() => this.props.navigation.navigate('AddPost',{groupId: this.props.navigation.getParam('id')})} style={{borderRadius:2, paddingVertical: 8, alignItems: 'center', backgroundColor: colors.primaryColor, width: '90%'}}>
                            <Text style={{color: colors.white, fontWeight: 'bold'}}>Add Post</Text>
                        </TouchableOpacity>
                    </View>
                    <View style={{flex: 1, alignItems: 'center'}}>
                        <TouchableOpacity onPress={() => this.props.navigation.navigate('ListChallenges',{groupId: this.props.navigation.getParam('id')})} style={{borderRadius:2, paddingVertical: 8, alignItems: 'center', backgroundColor: "orange", width: '90%'}}>
                            <Text style={{color: colors.white, fontWeight: 'bold'}}>Challenge</Text>
                        </TouchableOpacity>
                    </View>
                </View>
               )}
               ...
               {this.state.isShowButtonJoin ? (<Text style={{marginLeft: 20, color: colors.darkGray, fontWeight: 'bold'}}>Join group to see more posts !</Text>): (
                       ...
                       <Card key={index} style={{borderBottomWidth: 1, paddingBottom: 5, borderBottomColor: colors.darkGray, marginBottom: 10}}>
                            <Card.Title
                                ...
                                (props) => (<TouchableOpacity onPress={() => 
                                                ...
                                                <Icon name='more' style={{marginRight: 20}}/>
                                            </TouchableOpacity>)
                                }
                            />
                            <Card.Cover source={{uri: item.image}} style={{paddingLeft: 10, paddingRight: 10, marginBottom: 10, height: metrics.DEVICE_HEIGHT*0.3}}/>
                            <Card.Content>
                   ...
                   {this.state.isShowButtonJoin ? null : 
                    (   
                        <View style={{alignItems: 'center', paddingVertical: 10, backgroundColor: colors.white}}>
                            <TouchableOpacity onPress={() => this.deleteJoin()} style={{borderWidth: 1, borderColor: colors.darkGray, borderRadius:2, paddingVertical: 8, alignItems: 'center', backgroundColor: colors.white, width: metrics.DEVICE_WIDTH*0.9}}>
                                <Text style={{color: colors.red, fontWeight: 'bold'}}>Leave Group</Text>
                            </TouchableOpacity>
                        </View>
                    ) 
                    }
