        ...
        return (
            <ScrollView>
                {this.state.groups.map((item, index) => (
                    ...
                    bottomDivider
                    chevron
                    onPress={() => this.props.navigation.navigate("ClubDetail",{id: item.id, image: item.image, countRunners: this.state.countRunners[item.id] ? this.state.countRunners[item.id]: 0})}
                    />
                ))}
            </ScrollView>
        )
