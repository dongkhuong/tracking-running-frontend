        ...
        return (
           <ScrollView style={{backgroundColor: '#E7EAED'}}                     
            ...
            />   
               ...
               <View style={{height: metrics.DEVICE_HEIGHT*0.2}}>
                    <Text style={{marginHorizontal: 10, marginVertical: 5,fontWeight: 'bold', color: colors.darkGray}}>YOUR GROUPS</Text>
                    <ScrollView horizontal={true} style={{backgroundColor: colors.white}} contentContainerStyle={{alignItems: 'center'}}>
                        {this.state.groupsOfUser.map((item, index) => (
                            <TouchableOpacity key={index} onPress={() => this.props.navigation.navigate("ClubDetail",{id: item.id, countRunners: this.state.countRunners[item.id] ? this.state.countRunners[item.id]: 0})}>
                            ...
                            return (
                                <TouchableOpacity key={index} onPress={() => this.props.navigation.navigate("ClubDetail",{id: item.id, countRunners: this.state.countRunners[item.id] ? this.state.countRunners[item.id]: 0})}>
                                    <View style={{ height: metrics.DEVICE_HEIGHT*0.25, width: metrics.DEVICE_HEIGHT*0.25, alignItems: 'center', borderWidth: 0.5, borderColor: colors.gray, marginHorizontal: 5, marginVertical: 5}}>
                                        ...
                                        <Text style={{fontSize: 13, textAlign: 'center', position: 'absolute', color: colors.darkGray, bottom: 10}}>{this.state.countRunners[item.id] ? this.state.countRunners[item.id]: 0} Runners</Text>
                                    </View>
                                </TouchableOpacity>
                            )
                        })}
