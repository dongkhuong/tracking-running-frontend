        ...
        {text: 'Yes', onPress: async () => {
          if(this.state.duration != null)
          {
              ...
              if(this.state.distanceTravelled != 0){
                let distance = parseFloat(this.state.distanceTravelled)
                const averagePaceDecimal = parseFloat(timeDecimal/distance)
  ...
  async componentDidMount() {
    ...
    try {
      ...
      if(granted == PermissionsAndroid.RESULTS.GRANTED) {
        await Geolocation.getCurrentPosition((position) => {this.setState({latitude: position.coords.latitude, longitude: position.coords.longitude})},{enableHighAccuracy: true, timeout: 2000, maximumAge: 0})
                ...
                <TouchableOpacity onPress={() => this.setState({isClickScreen: !this.state.isClickScreen})} style={{height: 'auto', alignItems: 'center', paddingBottom: 20}}>
                    ...
                    <View style={{flexDirection: 'row'}}>
                        ...
                        <Right style={{alignItems: 'center'}}>
                            ...
                            <Text style={{fontSize: 20, fontWeight: 'bold'}}>{this.state.distanceTravelled}</Text>
