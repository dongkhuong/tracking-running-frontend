...
import { View, Text, TouchableOpacity, Platform, Image, TouchableHighlight, Alert } from "react-native"
...
class Activity extends React.Component {
        ...
        {text: 'Yes', onPress: () => {
          if(this.state.duration != null)
          {
              ...
              if(this.state.distanceTravelled != 0){
                ...
                const averagePace = parseFloat(timeDecimal/distance).toFixed(2)
                let m = moment().startOf('day').add(averagePace, 'hours').format('HH:mm')
                console.log(averagePace + "///"+ m)
                // this.setState({averagePace: moment.duration(averagePace, "minutes")})
                // console.log(this.state.averagePace) 
              } else {
                this.setState({averagePace: "00:00"})
                console.log(this.state.averagePace)
              }
          ...
          //   calories: 10,
          //   average_pace: "00:10:05"
            
          // }
          // this.props.navigation.navigate("CreatePost", {dataActivity: dataActivity})
        }},
        {
          text: 'No', onPress: () => {
  ...
  calcAverageSpeed = (time, distance) => {
  ...
  componentDidMount() {
    ...
    setInterval(() => this.setState({timerChange: this.currentTime}), 1000)
    // if(this.state.timerChange !=null) {
      console.log(this.state.timerChange)
    // }
  }
  ...
  render() {
    return (
           <View style={{flex: 1}}>
              <View style={{flex:1, borderBottomWidth: 1, borderBottomColor: colors.gray}}>
              ...
              <View style={this.state.isClickScreen == false ? {flex: 2}: {flex:4}}>
