        ...
        {text: 'Yes', onPress: () => {
          if(this.state.duration != null)
          {
              ...
              if(this.state.distanceTravelled != 0){
                let distance = parseFloat(this.state.distanceTravelled).toFixed(3)
                const averagePace = parseFloat(timeDecimal/distance).toFixed(3)
                const averageSpeed = parseFloat(60/averagePace)
                let m = moment().startOf('day').add(averagePace, 'hours').format('HH:mm')
                console.log(averagePace + "///"+ m + "///" + averageSpeed)
                ...
                <TouchableOpacity onPress={() => this.setState({isClickScreen: !this.state.isClickScreen})} style={{height: 'auto', alignItems: 'center', paddingBottom: 20}}>
                    ...
                    <View style={{flexDirection: 'row'}}>
                        ...
                        <Right style={{alignItems: 'center'}}>
                            <Text style={{fontSize: 20, fontWeight: 'bold'}}>{parseFloat(this.state.distanceTravelled).toFixed(3)}</Text>
