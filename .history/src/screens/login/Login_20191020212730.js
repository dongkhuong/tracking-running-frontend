...
class Login extends Component {
    ...
    _signInAsync = async () => {
        if(this.state.email!='' && this.state.password!='') {
            ...
            .then(async (response) => {
                let token = await AsyncStorage.setItem('token',response.data.data.token)
                let currentUser = await AsyncStorage.setItem('currentUser',JSON.stringify(response.data.data.user.email))
                console.log(token)
            })
